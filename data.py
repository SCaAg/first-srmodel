...
class DIV2K:
    def __init__(self,
        ...
        if subset == 'train':
            self.image_ids = range(1, 66852)
        elif subset == 'valid':
            self.image_ids = range(66852, 72709)
        else:
    ...
    def _hr_image_files(self):
        ...
        return [os.path.join(images_dir, f'{image_id:06}.png') for image_id in self.image_ids]
    ...
    def _lr_image_file(self, image_id):
        if not self._ntire_2018 or self.scale == 8:
            return f'{image_id:06}x{self.scale}.png'
        else:
            return f'{image_id:06}x{self.scale}{self.downgrade[0]}.png'
